...
function PrivacyNotice({ privacyNotice }: privacyNoticeProps) {
  return (
    <>
      ...
      <Container>
        <ContentWrapper>
          <Divider>Datenschutzerklärung</Divider>
          <Content>
            <Markdown
              components={{
                h3: ({ children }: any) => (
                  <HeadlineItemThree headline={3}>{children}</HeadlineItemThree>
                ),
                h4: ({ children }: any) => (
...
export async function getStaticProps() {
  const responsePrivacyNotice = await apiFetch("/datenschutzerklaerung");
